...
import os
import sys
sys.path.append('/home/jhpark/PycharmProjects/NWPparsing')
import CONSTANT
...
def extract_value(nearest_type=1):
    ...
    def base_setting(grid_analyzer):
        ...
        if grid_analyzer.set_lat_lon_call % 100 == 0:
            ...
            lat = lat_lon_grid[0]
            lon = lat_lon_grid[1]
            grid_analyzer.set_lat_lon_grid(lat, lon)

            # grid_analyzer.dset_lat_lon_grid(
    ...
    if file.variables == "all":
        file.variables = pd.read_excel(CONSTANT.setting_file_path + file.info_file_name).set_index(
    ...
    for i, var_name in enumerate(file.variables):
        ...
        for i, point in enumerate(file.location_points):
            if nearest_type != 1:
                ...
                #     grid_analyzer.nearest_n_grid_point(
                #         point[0], point[1], nearest_type)
                value = grid_analyzer.nearest_n_point_weighted_value(
...
@profile
def extract_value2(nearest_type=1):
    df = None
